    ...
    private class NetHandler extends PacketInboundHandler {
        ...
        protected void channelRead0(ChannelHandlerContext ctx, Packet packet) {
            if (packet instanceof P22_UpdateChannelDataRequest) {
                ...
                MCUProtocolV2.sendPacket(ctx, new P23_UpdateChannelList(server().fileService()
                                                                    .sources()
                                                                    .values()
                                                                    .stream()
                                                                    .map(FileSource::meta)
            ...
            if (packet instanceof P20_VersionLogRequest p) {
                ...
                MCUProtocolV2.sendPacket(ctx, new P21_VersionLogInfo(server().versionLog(p.getVersion())));
            ...
            if (packet instanceof P16_CDNDownloadComplete cc) {
                ...
                if(waitingList.containsKey(id)) {
                    MCUProtocolV2.sendPacket(ctx, waitingList.get(id));

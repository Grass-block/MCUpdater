...
public final class MCUpdaterServer {
    ...
    private void sendAddFileResponse(ArrayList<pair> jobs, ChannelHandlerContext ctx) throws Exception {
        MCUProtocolV2.sendPacket(ctx, new P0F_ServerProgressUpdate("正在准备文件..."));
        ...
        MCUProtocolV2.sendPacket(ctx, new P1F_UpdateProgressPredict(totalSize));
        ...
        for (var data : jobs) {
            ...
            try (var in = new FileInputStream(file)) {
                ...
                if (size > 262144) {
                    ...
                    MCUProtocolV2.sendPacket(ctx, packet);
    ...
    private String sendResourceFileResponse(ArrayList<String> resourcePacks, ChannelHandlerContext ctx) throws Exception {
        MCUProtocolV2.sendPacket(ctx, new P0F_ServerProgressUpdate("准备下载资源包..."));
        ...
        MCUProtocolV2.sendPacket(ctx, new P15_CDNDownloads(id, enableCDN ? ip : "_", port, this.cdn.getRepository(), cdnPacks));
        return id;
    }
    ...
    private void sendPatchFile(File file, ChannelHandlerContext ctx) throws Exception {
        ...
        MCUProtocolV2.sendPacket(ctx, new P14_PatchFileSlice(new byte[0], org.atcraftmc.updater.network.packet.P14_PatchFileSlice.SIG_END));
    }
    ...
    public void sendUpdateDataResponse(org.atcraftmc.updater.network.packet.P10_VersionInfo vi, ChannelHandlerContext ctx) {
        ...
        try {
            MCUProtocolV2.sendPacket(ctx, new P0F_ServerProgressUpdate("正在合并版本信息..."));
            ...
            MCUProtocolV2.sendPacket(ctx, new P0F_ServerProgressUpdate("正在分析版本并移除过时的文件..."));
            ...
            for (var v : vi.getInfos()) {
                ...
                MCUProtocolV2.sendPacket(ctx, new P12_FileDelete(version.remove().toArray(new String[0])));
            ...
            if (addFiles.isEmpty()) {
                MCUProtocolV2.sendPacket(ctx, new P0F_ServerProgressUpdate("未发现文件更新:("));
            } else {
                this.sendAddFileResponse(addFiles, ctx);
            }

            if (resourcePacks.isEmpty()) {
                MCUProtocolV2.sendPacket(ctx, new P0F_ServerProgressUpdate("未发现资源包更新:("));
                MCUProtocolV2.sendPacket(ctx, new P10_VersionInfo(result));
            } else {

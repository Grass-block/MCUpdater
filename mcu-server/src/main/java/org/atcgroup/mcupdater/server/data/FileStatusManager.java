package org.atcgroup.mcupdater.server.data;
...
public final class FileStatusManager {
    ...
    public FileModifyStatus checkFileStatus(String path) {
        ...
        if (!sha.exists() || sha.length() == 0) {
            ...
            //LOGGER.info(I18n.message("file.status.directory_created", sha.getParentFile().getAbsolutePath()));
        ...
        try (var in = new FileInputStream(sha)) {
            ...
            if (in.read(bytes) < 32) {
                ...
                LOGGER.error(I18n.message("file.status.sha_incorrect", sha.getAbsolutePath()));
            }
